...
By default, the tolerance is equal to `2.5e-3`, but can be adjusted using the argument `confidence.tol`.
...
In the example above, the user is informed that the estimation of some of the confidence intervals have been terminated before the p-value at the endpoints are within tolerance to the size of the test since the iteration limit was reached.
The estimated confidence interval may therefore not reflect the confidence level set by the user.
The actual p-values at the end points of each confidence interval is returned in the entry `$ci` of the output, along with additional details on the iterative procedure.
...
This implies that the solver is unable to perform the test at the level of precision requried for the p-value of the bound to fall within tolerance to the size of the test.
...
However, since the limitation pertains to the solver, there is no guarantee this will improve the estimate of the confidence intervals.
